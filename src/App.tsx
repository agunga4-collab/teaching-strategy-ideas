        ...
        <AnimatePresence mode="wait">
          {view === 'home' && (
            <motion.div key="home" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
              <div className="mb-8">
                <h2 className="text-3xl font-bold text-stone-800 mb-2">Jelajahi Rutinitas Berpikir</h2>
                <p className="text-stone-600">Pilih kategori atau <span className="font-semibold cursor-pointer text-stone-800 underline" onClick={goAsk}>Tanya AI</span> untuk rekomendasi.</p>
              </div>
          ...
          {view === 'routine' && selectedRoutine && selectedCategory && (
            <motion.div key="routine" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
              <button onClick={() => { setView('category'); setSelectedRoutine(null); window.scrollTo(0,0); }} className="mb-4 text-stone-500 hover:text-stone-700">← Kembali ke {selectedCategory.name}</button>
